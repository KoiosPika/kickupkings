...
const ShopPage = () => {
    ...
    return (
        <section className='w-full h-screen'>
            ...
            <div className='w-full ml-auto mb-auto p-2 flex flex-row items-center gap-2'>
                <div className='w-1/2 bg-slate-500 flex flex-col justify-center items-center rounded-lg h-[53px] sm:h-[75px] gap-[3px]'>
                    <p className='font-bold text-white text-[16px] sm:text-[22px]'>Points: 2.4k / 5k</p>
                    <div className='w-11/12 flex flex-row items-center'>
                        <div className='h-[5px] sm:h-[10px] w-8/12 rounded-l-lg bg-orange-500' />
                        <div className='h-[5px] sm:h-[10px] w-4/12 rounded-r-lg bg-white' />
                    </div>
                </div>
                <div className='w-1/2 bg-slate-500 flex flex-col justify-center items-center rounded-lg h-[53px] sm:h-[75px]'>
            ...
            <div className='w-full flex flex-col justify-center items-center my-2'>
                ...
                <div className='w-11/12 bg-slate-500 flex flex-row items-center justify-evenly px-1 py-1 rounded-lg my-2'>
                    {['Defense', 'Midfield', 'Forward', 'Staff'].map((type) => (
                        <p
                            key={type}
                            className={`w-1/4 rounded-md text-center text-[12.5px] sm:text-[24px] font-semibold py-[2px] sm:py-[7px] cursor-pointer text-white ${selectedType === type ? 'bg-[#09C609] text-white' : ''
                                }`}
                ...
                <ScrollArea className='w-11/12' style={{ height: height - 355 }}>
                    <div className='grid grid-cols-2 w-full gap-2 sm:gap-3'>
                        {positions
                            .filter((position) => position.type === selectedType)
                            .map((position: any, index: number) => (
                                <div key={index} className='flex flex-col justify-center items-center w-full bg-slate-500 rounded-xl h-[100px] sm:h-[150px]'>
                                    <div className='flex flex-row items-center w-full p-2 gap-1'>
                                        <div className={`p-2 rounded-md font-bold w-1/4 sm:w-1/3 text-center text-white h-full flex justify-center items-center text-[13.5px] sm:text-[25px] border-2 border-white`} style={{ backgroundColor: position.color, boxShadow: `-8px -8px 10px -6px ${position.color},-8px 8px 10px -6px ${position.color},8px -8px 10px -6px ${position.color},8px 8px 10px -6px ${position.color}` }}>
                                            <p>{position.symbol}</p>
                                        </div>
                                        <div className='flex flex-col justify-center items-center w-3/4 sm:w-2/3 gap-1'>
                                            <p className='text-[11px] sm:text-[18.5px] font-semibold text-white'>{position.label}</p>
